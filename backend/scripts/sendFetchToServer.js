...
const sendFetchToServer = async (res, courtsIdx, cookieStr) => {
  ...
  const bodyStr = `Reservation_Date=${dateStrings[0]}%2F${dateStrings[1]}%2F${dateStrings[2]}&Reservation_Num=&LastBlock=44&Mode=New&From=Reservations&Player_1=RANDI.HEDBERG&Court_Num=${res.courts[courtsIdx]}&Start_Time=${res.time}&Duration=2&Extended_Desc=`;
  const headers = {
